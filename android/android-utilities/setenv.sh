...
if [ $(ls -1 "$ANDROID_SDK_ROOT/build-tools" | wc -l) -ge 1 ] ; then
...
if [ $(ls -1 "$ANDROID_SDK_ROOT/ndk" | wc -l) -ge 1 ] ; then
	export ANDROID_NDK=$(ls -1 $ANDROID_SDK_ROOT/ndk | grep "^[0-9]" | tail -1)
	echo "Using discovered ndk version $(basename $ANDROID_NDK)"
	export ANDROID_NDK_ROOT=$ANDROID_NDK
fi
