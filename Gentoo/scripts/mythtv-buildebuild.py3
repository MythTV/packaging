...
# Set Global Timeout for socket connections
gvar_timeout = 30 # Timeout in Seconds
retryLimit=11 # Retry Limit
...
def get_urlReadDecodeWrapper(url):
	for x in range(retryLimit): 
		if x == retryLimit-1: # Limit the number of retries to 10
			print('Too Many Unsuccessful Server Attempts: Exiting.')
			exit(1)
			
		try:
			readUrl = urllib.request.urlopen(url,timeout=gvar_timeout).read()
		#urllib.request can throw 3 different exceptions: reason, socket.error, OSError
		except urllib.error.URLError as e:
			print (e.reason)
			# Capture Reason Exception for URLError
			if (str(e.reason) == 'timed out'):
				print('Errno {0}: Connection Timeout: Retrying {1} of {2} in {3} seconds.'.format(e.errno,x+1,retryLimit-1,gvar_timeout))
			else:
				raise # re-raise any other error
		except IOError as e:
			# Capture socket.error as an IOError 
			if (e.errno == errno.ETIMEDOUT):
				print('Errno {0}: Connection Timeout: Retrying {1} of {2} in {3} seconds.'.format(e.errno,x+1,retryLimit-1,gvar_timeout))
			elif (e.errno == errno.ECONNRESET):
				print('Errno {0}: Connection Reset by Peer: Retrying {1} of {2} in {3} seconds.'.format(e.errno,x+1,retryLimit-1,gvar_timeout))
			elif (e.errno == errno.ECONNREFUSED):
				print('Errno {0}: Connection Refused by Peer: Retrying {1} of {2} in {3} seconds.'.format(e.errno,x+1,retryLimit-1,gvar_timeout))
			elif (e.errno == errno.ECONNABORTED):
				print('Errno {0}: Connection Aborted by Peer: Retrying {1} of {2} in {3} seconds.'.format(e.errno,x+1,retryLimit-1,gvar_timeout))
			else:
				raise # re-raise any other error
		except OSError:
			# Capture OSError 
			...
			break # response successful
		time.sleep(gvar_timeout) # Sleep for gvar_timeout

	return readUrl.decode('utf-8')
	
def get_urlHeaderWrapper(url):
	request = urllib.request.Request(url)
	opener = urllib.request.build_opener()
	headers=''
	for x in range (retryLimit): # Limit the number of retries to 10
		if x == retryLimit-1: # Limit the number of retries to 10
			print('Too Many Unsuccessful Server Attempts: Exiting.')
			exit(1)
		
		try:
			f = opener.open(request,timeout=gvar_timeout)
...
def get_urlFileWrapper(url, localfile):
	for x in range (retryLimit): # Limit the number of retries to 10
		if x == retryLimit-1: # Limit the number of retries to 10
			print('Too Many Unsuccessful Server Attempts: Exiting.')
			exit(1)
...
class Ebuild( object ):
	...
	def get_tarball(self):
		...
		# get the headers
		headers=get_urlHeaderWrapper(url)

		for line in headers:
			line=str(line)
			if (line.startswith('(\'Content-Length')) :
				line = multi_split(line.strip(), ' \',()')
				tarBallsize=int(line[1])
				if self.opts.verbose: print('Mythtv Download Size: ' + str(tarBallsize))
		...
		if (os.access(self.opts.tarball, os.F_OK) and (os.path.getsize(self.opts.tarball) == tarBallsize)) :
			# tarball already exists
			if self.opts.verbose: print( 'Downloading "{0}" to "{1}"'.format(url, self.opts.tarball))
			if self.opts.verbose: print( 'Download Complete'.format(url, self.opts.tarball))
			return

		if self.opts.verbose: print( 'Downloading "{0}" to "{1}"'.format(url, self.opts.tarball))
		
		get_urlFileWrapper(url, self.opts.tarball)
	...
	def get_shorthash(self):
		...
		self.opts.shash = tar.stdout.readline().strip().strip(b"/").split(b"-")[-1].decode('utf-8')
		tar.stdout.close()
		tar.wait()
	...
	def get_cur(self):
		...
		if type != 4:
			if self.opts.hash is None:
				commits = json.loads(get_urlReadDecodeWrapper("http://github.com/api/v2/json/commits/list/mythtv/MythTV/"+self.branch))
				...
				commit = json.loads(get_urlReadDecodeWrapper("http://github.com/api/v2/json/commits/list/mythtv/MythTV/"+self.opts.hash))
	...
	def copy_base(self):
		...
		if base == cur:
		# The base file and current file are the same no work to be done!
			return
	...
	def digest(self, verbose=False, force=False):
		...
		if self.opts.verbose: 
			nicePrintCmd=''
			for line in cmd:
				nicePrintCmd=nicePrintCmd + ' ' + str(line)
			print( 'Digesting:',str(nicePrintCmd))
